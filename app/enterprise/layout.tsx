...
function EnterpriseLayoutContent({ children }: { children: React.ReactNode }) {
  ...
  const getAccessibleModules = () => {
    const allModules = [
      { label: 'HR Management', href: '/enterprise/hr', icon: '👥', roles: ['SUPERADMIN', 'ADMIN', 'SUPERVISOR'], description: 'Manage employees, timesheets, attendance' },
      { label: 'Task Management', href: '/enterprise/tasks', icon: '✓', roles: ['SUPERADMIN', 'ADMIN', 'SUPERVISOR', 'INTERN'], description: 'Assign and track tasks' },
      { label: 'Products & Inventory', href: '/enterprise/inventory', icon: '📦', roles: ['SUPERADMIN', 'ADMIN'], description: 'Manage product catalog and stock' },
      { label: 'Sales Management', href: '/enterprise/sales', icon: '💼', roles: ['SUPERADMIN', 'ADMIN', 'SUPERVISOR'], description: 'Sales reports and pipeline tracking' },
      { label: 'POS System', href: '/enterprise/pos', icon: '💳', roles: ['SUPERADMIN', 'ADMIN'], description: 'Point of Sale operations' },
      { label: 'Intern Management', href: '/enterprise/interns', icon: '🎓', roles: ['SUPERADMIN', 'ADMIN', 'SUPERVISOR'], description: 'Monitor intern progress and feedback' },
      { label: 'Reports & Analytics', href: '/enterprise/reports', icon: '📊', roles: ['SUPERADMIN', 'ADMIN'], description: 'Business intelligence and reporting' },
    ];
    ...
    <motion.div className="min-h-screen bg-black flex" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      >
          ...
          >
            <div className="flex items-start gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-bold text-yellow-400">{getRoleDescription()}</p>
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-2">
            ...
            {modules.map((module, idx) => (
              <Link key={idx} href={module.href}>
                <motion.div
                  ...
                  whileHover={{ x: 4 }}
                  whileTap={{ scale: 0.98 }}
                  title={isSidebarOpen ? '' : module.description}
                >
          ...
          {isSidebarOpen && modules.length > 0 && (
          ...
          {isSidebarOpen && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
              ...
              <div className="space-y-1">
                <Link href={userRole === 'INTERN' ? '/account/intern-dashboard' : '/account/profile'}>
                  <motion.div
                    className={`px-4 py-3 rounded-lg border transition-all cursor-pointer flex items-center gap-3 ${
                      isActive('/account')
                    ...
                    whileTap={{ scale: 0.98 }}
                  >
                    <span className="text-lg flex-shrink-0">{userRole === 'INTERN' ? '📊' : '⚙️'}</span>
                    <span className="text-sm font-medium">{userRole === 'INTERN' ? 'Dashboard' : 'Settings'}</span>
                    {isActive('/account') && (
                      <motion.div
          ...
          {modules.length === 0 && isSidebarOpen && (
            <motion.div
              className="px-4 py-6 text-xs text-gray-500 text-center italic"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            >
        ...
        {isSidebarOpen && (
          ...
          >
            ...
            <p className="text-xs text-gray-600 text-center mt-3">v1.0 • Enterprise</p>
          </motion.div>
        )}
      </motion.div>
      ...
      <div className="flex-1 flex flex-col">
        ...
        >
          <h1 className="text-2xl sm:text-3xl font-bold text-white">
            Enterprise <span className="text-yellow-400">Management</span>
          </h1>
        </motion.div>
...
export default function EnterpriseLayout({ children }: { children: React.ReactNode }) {
  return (
    <ProtectRoute requiredRoles={['SUPERADMIN', 'ADMIN', 'SUPERVISOR', 'INTERN']}>
      <EnterpriseLayoutContent>{children}</EnterpriseLayoutContent>
    </ProtectRoute>
  );
}
